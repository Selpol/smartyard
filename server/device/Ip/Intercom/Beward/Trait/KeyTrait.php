...
trait KeyTrait
{
    ...
    public function getKeys(?int $apartment): array
    {
        if ($this->model->mifare) {
            $response = $this->parseParamValueHelp($this->get('/cgi-bin/mifare_cgi', ['action' => 'list'], parse: false));
        } else {
    ...
    {
        if ($this->model->mifare) {
            $this->get('/cgi-bin/mifare_cgi', ['action' => 'add', 'Key' => $key->key, 'Apartment' => $key->apartment, 'CipherIndex' => 1]);
        } else {
