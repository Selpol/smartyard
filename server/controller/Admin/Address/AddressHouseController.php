...
readonly class AddressHouseController extends AdminRbtController
{
    ...
    public function magic(AddressHouseMagicRequest $request, GeoFeature $feature): ResponseInterface
    {
        ...
        if ($house["data"]["area_fias_id"]) {
            $area = AddressArea::fetch(criteria()->nullable()->equal('area_uuid', $house['data']['area_fias_id'])->equal('address_region_id', $regionId)->equal('area', $house['data']['area']));
        ...
        if ($house["data"]["city_fias_id"]) {
            $city = AddressCity::fetch(criteria()->nullable()->equal('city_uuid', $house['data']['city_fias_id'])->equal('address_region_id', $regionId)->equal('address_area_id', $areaId)->equal('city', $house['data']['city']));
        ...
        if ($house["data"]["settlement_fias_id"]) {
            $settlement = AddressSettlement::fetch(criteria()->nullable()->equal('settlement_uuid', $house['data']['settlement_fias_id'])->equal('address_area_id', $areaId)->equal('address_city_id', $cityId)->equal('settlement', $house['data']['settlement']));
        ...
        if ($house["data"]["street_fias_id"]) {
            $street = AddressStreet::fetch(criteria()->nullable()->equal('street_uuid', $house['data']['street_fias_id'])->equal('address_city_id', $cityId)->equal('address_settlement_id', $settlementId)->equal('street', $house['data']['street']));
        ...
        if ($house["data"]["house_fias_id"]) {
            ...
            if (!$addressHouse) {
                ...
                $addressHouse->house_full = $house['data']['house_full'];
